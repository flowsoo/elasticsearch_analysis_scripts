...
#
# Optional:
#   ES_URL=http://localhost:9200 ./es-memory-check.sh
...
echo "============================================================"
echo " Elasticsearch Memory Diagnostic"
echo "============================================================"
echo
echo "Time:    $(date)"
echo "Host:    $(hostname)"
...
ES_INFO="$(curl -s --max-time "$CURL_TIMEOUT" "$ES_URL/" 2>/dev/null || true)"

if [ -z "$ES_INFO" ]; then
    fail "Could not connect to Elasticsearch at $ES_URL"
    echo
    echo "Check that Elasticsearch is running and that ES_URL is correct."
    ES_AVAILABLE=0
else
...
if [ "$ES_AVAILABLE" -eq 1 ]; then
    ...
    JVM_JSON="$(curl -s --max-time "$CURL_TIMEOUT" \
        "$ES_URL/_nodes/stats/jvm" 2>/dev/null || true)"
...
' 2>/dev/null || {
            warn "Python JSON parser unavailable; raw JVM response:"
            echo "$JVM_JSON"
        }
    fi
fi
...
if [ "$ES_AVAILABLE" -eq 1 ]; then
    ...
    INDICES="$(curl -s --max-time "$CURL_TIMEOUT" \
        "$ES_URL/_cat/indices?v&bytes=gb&s=store.size:desc" 2>/dev/null || true)"
    ...
    SHARDS="$(curl -s --max-time "$CURL_TIMEOUT" \
...
if [ "$ES_AVAILABLE" -eq 1 ]; then
    ...
    INDICES_STATS="$(curl -s --max-time "$CURL_TIMEOUT" \
        "$ES_URL/_nodes/stats/indices" 2>/dev/null || true)"
...
' 2>/dev/null || {
            warn "Could not parse index statistics."
        }
...
if [ "$ES_AVAILABLE" -eq 1 ]; then
    ...
    BREAKERS="$(curl -s --max-time "$CURL_TIMEOUT" \
        "$ES_URL/_nodes/stats/breaker" 2>/dev/null || true)"
...
try:
    ...
    for node_id,node in d.get("nodes",{}).items():
        ...
        for name,b in breakers.items():
            ...
            print("  %-20s estimated=%8.2f GB limit=%8.2f GB trips=%d" %
                  (name,estimated/1024**3,limit/1024**3,trips))
...
if [ "$ES_AVAILABLE" -eq 1 ]; then
    ...
    INDEX_NAMES="$(curl -s --max-time "$CURL_TIMEOUT" \
        "$ES_URL/_cat/indices?h=index&s=index" 2>/dev/null || true)"
    ...
    if [ -n "$INDEX_NAMES" ]; then
        ...
        MATCHES="$(echo "$INDEX_NAMES" | grep -Eic \
            '20[0-9]{2}[.-][0-9]{2}[.-][0-9]{2}|[0-9]{8}|daily|day-|date-|logs-|logstash-|metrics-|filebeat-|metricbeat-' || true)"
...
if [ "$ES_AVAILABLE" -eq 1 ]; then
    ...
    SETTINGS="$(curl -s --max-time "$CURL_TIMEOUT" \
